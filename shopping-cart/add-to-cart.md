...
Specifying `"_update_quantity": 1` in the request body lets you update the existing line item quantity \(if any\) instead of creating a new line item for the same SKU. That means:
...
{% hint style="warning" %}
If you send the POST request without the `"_update_quantity": 1` param a new line item is always created, even if the line item SKU is already present in the shopping cart.
{% endhint %}
